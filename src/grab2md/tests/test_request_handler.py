...
from grab2md.network.request_handler import FetchResult, fetch_html


@pytest.fixture(autouse=True)
def route_mock_sessions_through_contract_boundary(monkeypatch):
    from grab2md.network import request_handler
...
def test_server_error_retries_then_returns_real_success():
    ...
    with patch("grab2md.network.request_handler.time.sleep") as sleep:
...
def test_local_server_5xx_retries_and_recovers():
    server, base_url = start_contract_server()
    try:
        with patch("grab2md.network.request_handler.time.sleep"):
            result = fetch_html(
                f"{base_url}/flaky",
                requests.Session(),
                {},
