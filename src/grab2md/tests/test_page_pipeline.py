...
from grab2md.markdown.pipeline import (
    AcquisitionFailure,
    AcquiredPage,
    PageConverter,
    acquire_http_page,
...
from grab2md.network.browser_renderer import RenderedPage
...
def test_static_acquisition_preserves_requested_and_final_representation_metadata():
    ...
    with patch(
        "grab2md.markdown.pipeline.guarded_request", return_value=_response()
    ) as request:
...
def test_static_acquisition_rejects_non_html_and_preserves_status_failures():
    session = requests.Session()
    with patch(
        "grab2md.markdown.pipeline.guarded_request",
    ...
    with patch("grab2md.markdown.pipeline.guarded_request", return_value=response):
...
def test_local_and_rendered_acquisition_share_the_page_contract(tmp_path):
    ...
    with patch(
        "grab2md.markdown.pipeline.render_html",
