...
from grab2md.cookies.session_manager import (
    CookieRecord,
    CookieSourceError,
...
from grab2md.cookies.firefox import find_firefox_profile
...
def test_browser_cookie_mapping_is_applied_to_target_host():
    ...
    with patch("grab2md.cookies.sources.browser_cookie_source", return_value=source):
...
def test_browser_cookie_mapping_honors_explicit_browser():
    ...
    source.load.return_value = {"session": "value"}
    with patch(
        "grab2md.cookies.sources.browser_cookie_source", return_value=source
    ) as select:
...
def test_domain_cookie_loader_routes_to_configured_browser():
    with (
        patch(
            "grab2md.cookies.sources.load_config",
            return_value={"browser": {"preferred": "firefox"}},
        ),
        patch(
            "grab2md.cookies.firefox.get_firefox_cookies",
            return_value=[
...
def test_browser_source_selection_reads_configuration_at_call_time():
    from grab2md.cookies.sources import ChromeCookieSource, FirefoxCookieSource
...
def test_cookie_module_does_not_load_configuration_during_import():
    import grab2md.config.loader as config_loader
    import grab2md.cookies.session_manager as module
...
def test_macos_chrome_source_fails_closed_before_cookie_decryption(tmp_path):
    from grab2md.cookies.sources import ChromeCookieSource
    import grab2md.cookies.session_manager as session_manager

    database = tmp_path / "Cookies"
    database.write_bytes(b"fixture")
    with (
        patch("grab2md.cookies.sources.sys.platform", "darwin"),
        patch(
            "grab2md.cookies.browser_paths.get_browser_cookie_path",
    ...
    with patch("grab2md.cookies.chrome.sys.platform", "darwin"):
...
def test_browser_cookie_default_paths_are_explicit_platform_contracts(
    ...
    with (
        patch("grab2md.cookies.browser_paths.load_config", return_value={}),
        patch("grab2md.cookies.browser_paths.sys.platform", platform),
        patch("grab2md.cookies.browser_paths.Path.home", return_value=tmp_path),
    ):
...
def test_chrome_capability_rejects_unimplemented_key_stores(
    ...
    database.write_bytes(b"fixture")
    with (
        patch("grab2md.cookies.sources.sys.platform", platform),
        patch(
            "grab2md.cookies.browser_paths.get_browser_cookie_path",
            return_value=database,
        ),
...
def test_windows_chrome_capability_probes_the_key_boundary(tmp_path):
    ...
    database.write_bytes(b"fixture")
    with (
        patch("grab2md.cookies.sources.sys.platform", "win32"),
        patch(
            "grab2md.cookies.browser_paths.get_browser_cookie_path",
            return_value=database,
        ),
...
def test_windows_chrome_key_decodes_and_removes_dpapi_prefix(tmp_path):
    ...
    with (
        patch("grab2md.cookies.chrome.sys.platform", "win32"),
        patch("grab2md.cookies.chrome.Path.home", return_value=tmp_path),
...
def test_firefox_lookup_enforces_boundaries_and_preserves_scope(tmp_path):
    ...
    with (
        patch(
            "grab2md.cookies.firefox.get_browser_cookie_path",
