...
from grab2md.markdown.crawler import CrawlResult, crawl_website
...
def test_convert_failure_exits_nonzero():
    with patch("grab2md.cli.cli.process_single_quiet", return_value=False):
...
def test_batch_total_failure_exits_nonzero_without_completion(tmp_path):
    ...
    with patch("grab2md.cli.cli.process_markdown_links", return_value=failed):
...
def test_batch_partial_failure_exits_nonzero_and_reports_durable_output(tmp_path):
    ...
    with patch("grab2md.cli.cli.process_markdown_links", return_value=partial):
        result = runner.invoke(
            app,
...
def test_state_export_import_and_missing_info_fail_nonzero(tmp_path):
    with patch(
        "grab2md.cli.state_commands.StateManager.export_state",
    ...
    with patch(
        "grab2md.cli.state_commands.StateManager.import_state",
        side_effect=ValueError("invalid state"),
    ):
        imported = runner.invoke(app, ["state", "import", str(tmp_path / "bad.json")])
    with patch("grab2md.cli.state_commands.StateManager.load_state", return_value=None):
...
def test_crawler_fetch_failure_uses_typed_failure_result(tmp_path):
    ...
    with patch("grab2md.markdown.crawler.fetch_html", return_value=failure):
...
def test_crawler_robots_denial_uses_same_result_contract(tmp_path):
    state_manager = StateManager(state_dir=tmp_path / "states")
    with patch("grab2md.markdown.crawler.RobotsChecker") as checker_type:
...
def test_crawl_failure_exits_nonzero_without_success_message(tmp_path):
    ...
    with patch("grab2md.cli.cli.crawl_website", return_value=failed):
        result = runner.invoke(
            app,
...
def test_crawl_success_reports_completion(tmp_path):
    ...
    with patch("grab2md.cli.cli.crawl_website", return_value=succeeded):
        result = runner.invoke(
            app,
            ["crawl", "https://example.com", "--output-dir", str(tmp_path), "--quiet"],
        )
