...
from grab2md.network.header_manager import (
    HeaderConfig,
    HeaderManager,
    format_http_date,
    parse_http_date,
...
def test_default_identity_uses_installed_package_version_without_request_context():
    ...
    assert headers["User-Agent"] == f"grab2md/{__version__}"
...
def test_contact_identity_is_explicit_and_does_not_impersonate_a_browser():
    ...
    assert user_agent.startswith(f"grab2md/{__version__}")
