"""
State persistence and resume capability for grab2md crawler.
...
class StateManager:
    ...
    def __init__(self, state_dir: Optional[Path] = None):
        ...
        Args:
            state_dir: Directory to store state files.
                      Defaults to ~/.grab2md/states
        """
        if state_dir is None:
            self.state_dir = Path.home() / ".grab2md" / "states"
        else:
            self.state_dir = Path(state_dir)

        self.state_dir.mkdir(parents=True, exist_ok=True)
        self.store = CrawlStateStore(self.state_dir)
    ...
    def create_new_state(
        self, start_url: str, output_dir: str, config: Dict[str, Any]
    ) -> CrawlState:
        ...
        self.current_state = CrawlState(
            start_url=start_url, output_dir=str(output_dir), config=_json_safe(config)
        )
    ...
    def load_state(self, crawl_id: str) -> Optional[CrawlState]:
        """Load and migrate existing crawl state through the state store."""
        self.current_state = self.store.load(crawl_id)
        return self.current_state
    ...
    def list_resumable_crawls(self) -> List[Dict[str, Any]]:
        """List resumable states through the persistence store."""
        return self.store.list_resumable()

    def clean_old_states(self, days: int = 30):
        """Remove persisted states older than the requested age."""
        return self.store.clean_older_than(days)

    def export_state(self, crawl_id: str, output_file: Path):
        """Export one validated state document."""
        self.store.export(crawl_id, Path(output_file))

    def import_state(self, input_file: Path) -> str:
        """Import, migrate, and persist a state under a new identity."""
        self.current_state = self.store.import_file(Path(input_file))
        return self.current_state.crawl_id
