...
from grab2md.config.schema import validate_and_merge
...
def get_config_path():
    ...
    if sys.platform == "win32":
        config_dir = (
            Path(os.getenv("APPDATA", Path.home() / "AppData" / "Roaming")) / "grab2md"
        )
    elif sys.platform == "darwin":  # macOS
        config_dir = Path.home() / "Library" / "Application Support" / "grab2md"
    else:  # Assume Linux/Unix
        config_dir = (
            Path(os.getenv("XDG_CONFIG_HOME", Path.home() / ".config")) / "grab2md"
        )
...
CONFIG_FILE = Path(os.getenv("GRAB2MD_CONFIG_PATH", get_config_path()))
...
def get_recovery_handler():
    ...
    if _recovery_handler is None:
        from grab2md.config.recovery import ConfigRecoveryHandler
