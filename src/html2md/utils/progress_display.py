...
class CrawlProgress:
    ...
    def _create_domain_panel(self, stats: Dict[str, Any]) -> Panel:
        ...
        if "domain_stats" in stats:
            ...
            table.add_row(f"Active Domains: {active_domains}", "", "", "")
            table.add_row(
                f"Domains in Backoff: {backoff_domains}", "", "", "[red]Waiting[/red]"
            )
    ...
    def show_completion(
        self, total_urls: int, total_time: float, success_count: int, error_count: int
    ):
        ...
        table.add_row(
            "Successful:", f"{success_count} ({success_count/total_urls*100:.1f}%)"
        )
...
def estimate_crawl_time(
    total_urls: int, rate_limit: Optional[int] = None, avg_response_time: float = 1.0
) -> float:
    """
