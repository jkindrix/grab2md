...
def get_conversation_html(url, session, headers):
    ...
    try:
        ...
        if response.status_code == 200 and len(response.text) > 1000:
            ...
            if response.status_code != 200:
                logger.warning(
                    f"Direct HTML request failed with status {response.status_code}"
                )
                ...
                if (
                    "log in" in response.text.lower()
                    or "sign in" in response.text.lower()
                ):
                    logger.warning(
                        "Response appears to be a login page - authentication failed"
                    )

            logger.debug(
                "Failed response body omitted; length=%s", len(response.text or "")
            )
    ...
    try:
        ...
        logger.debug(
            "Session has %s cookies for API request", len(session.cookies.get_dict())
        )
        ...
        logger.debug(
            "API response body omitted; length=%s", len(api_response.text or "")
        )
        ...
        if api_response.status_code == 200:
            ...
            try:
                ...
                if "mapping" in data:
                    ...
                    for msg_id, msg_data in messages.items():
                        ...
                        if "message" in msg_data and msg_data.get("message"):
                            ...
                            if "parts" in content and content["parts"]:
                                logger.debug(
                                    f"Found content parts for {author} message"
                                )
                    ...
                    conversation = sorted(
                        conversation, key=lambda x: x.get("timestamp", "")
                    )
                    logger.debug(
                        f"Sorted conversation has {len(conversation)} messages"
                    )
                ...
                html += "</body></html>"
                logger.info(
                    f"Generated HTML for ChatGPT conversation with {len(conversation)} messages"
                )
                return html
            ...
            if api_response.status_code == 401:
                ...
                logger.error(
                    "Conversation not found. It might have been deleted or you don't have access to it."
                )
    ...
    try:
        ...
        if alt_response.status_code == 200:
            ...
            logger.warning(
                f"Alternate API request failed with status {alt_response.status_code}"
            )
            logger.debug(
                "Alternate response body omitted; length=%s",
                len(alt_response.text or ""),
            )
    except Exception as e:
            ...
            if access_token:
                ...
                if token_response.status_code == 200:
                    ...
                    try:
                        ...
                        html += (
                            f"<pre>{json.dumps(conversation_data, indent=2)}</pre>\n"
                        )
                        html += "</body></html>"
                        return html
                    except Exception as e:
                    ...
                    logger.warning(
                        f"Conversation API with token failed: {token_response.status_code}"
                    )
    except Exception as e:
