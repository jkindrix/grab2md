...
class ConcurrentLimiter:
        ...
        with self._lock:
            with self._lock:
                ...
                if state.active_connections == 0 and self.global_active == 0:
                    ...
                    logger.debug(
                        f"Acquired slot for {domain} ({state.active_connections} active)"
                    )
                    return True
                else:
                    return False
        ...
        with self._lock:
            with self._lock:
                ...
                if not success or (status_code and status_code >= 400):
                    ...
                    logger.info(
                        f"Error for domain {domain}: status={status_code}, "
                    ...
                    if status_code == 429:
                        ...
                        if retry_after and self.config.retry_after_respect:
                            ...
                            logger.warning(
                                f"Domain {domain} requested retry after {retry_after}s"
                            )
                        else:
                        ...
                        >= self.config.error_threshold_for_backoff
                    ):
                        self._apply_backoff(domain, state)
                else:
                    # Reset on success
                    if state.consecutive_errors > 0:
                        logger.info(
                            f"Domain {domain} recovered after {state.consecutive_errors} errors"
                        )
                ...
                logger.debug(
                    f"Released slot for {domain} ({state.active_connections} active)"
                )
    ...
    def _apply_backoff(self, domain: str, state: DomainState):
        ...
        logger.warning(
            f"Applying {backoff_time:.1f}s backoff to domain {domain} "
    ...
    def get_domain_stats(self, domain: str) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "in_backoff": bool(
                    state.backoff_until and time.time() < state.backoff_until
                ),
                "backoff_remaining": max(
                    0, (state.backoff_until - time.time()) if state.backoff_until else 0
                ),
                "error_rate": (
                    state.total_errors / state.total_requests * 100
