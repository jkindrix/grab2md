    ...
    def record_failure(self) -> None:
        ...
        with self.lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                logger.warning("Circuit breaker failed during testing, returning to OPEN")
    ...
    def can_make_request(self) -> Tuple[bool, float]:
        ...
        with self.lock:
            ...
            if current_count >= max_requests:
                ...
                oldest_request = self.counter.requests[0] if self.counter.requests else current_time
...
class GlobalRateLimiter:
    ...
    def record_request_end(self, url: str, start_time: float, success: bool,
                          response_time: Optional[float] = None) -> None:
        """Record the completion of a request."""
        limiter = self._get_domain_limiter(url)
    ...
    def get_all_stats(self) -> Dict[str, RateLimitStats]:
        ...
        with self.lock:
            return {domain: limiter.get_stats() 
