...
# This script demonstrates how to access ChatGPT data via their API
# It requires a valid access token extracted from browser cookies

# You'll need to replace this with your actual token value
# This is ONLY the token value, not the entire cookie

# First, try to get a token from the session token
def get_access_token(session_token):
    session = requests.Session()
    
    # Set up the request to the auth endpoint
...
if access_token:
    ...
    # Create a new session for the API request
    session = requests.Session()
    
    # Set up the conversation request
    conversation_id = "6812d27d-6498-8006-9c6e-e6b6a4d6c0eb"
    conversation_url = f"https://chat.openai.com/backend-api/conversation/{conversation_id}"
    ...
    # Make the request
    response = session.get(conversation_url, headers=headers)
    ...
    if response.status_code == 200:
        ...
        with open("token_api_response.json", "w") as f:
            f.write(response.text)

        # Save API response info to a separate file
        with open('token_api_info.txt', 'w') as f:
            ...
            f.write(f'\nAccess Token Used: {access_token[:15]}...\n')

        print(f"Saved conversation data to token_api_response.json")
        print(f"Response info saved to token_api_info.txt")
        
        # Parse and show sample of the conversation
        try:
            data = response.json()
            print(f"\nConversation data retrieved successfully. Sample:")
            if "mapping" in data:
                message_count = len(data["mapping"])
                print(f"Found {message_count} messages in the conversation")
            else:
        ...
        print(f"Failed to get conversation: {response.text[:200]}...")
else:
